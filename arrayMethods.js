...
// CODE HERE
const postTaxPrices  = prices.map(elem => (elem * 1.07).toFixed(2));
...
// CODE HERE
const ordersTaxed = orders.map(elem => (elem.price * (elem.tax + 1)).toFixed(2));
...
// CODE HERE
const bobsTotal = purchases.map(elem => elem.price).reduce((a, c) => a +c);
